    ...
    <div className="space-y-8 animate-in fade-in duration-500">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          ...
          <p className="text-slate-500 mt-1">Join global subjects or enter a code from your teacher.</p>
        </div>
        <div className="flex gap-3 w-full md:w-auto">
             <div className="bg-white border border-slate-200 rounded-xl p-1 flex items-center shadow-sm">
                <button 
                    onClick={() => setActiveTab('browse')}
                    className={`px-4 py-2 rounded-lg text-sm font-bold transition-all ${activeTab === 'browse' ? 'bg-indigo-600 text-white shadow-sm' : 'text-slate-500 hover:text-slate-700'}`}
                >
                    Browse All
                </button>
                <button 
                    onClick={() => setActiveTab('my-classes')}
                    className={`px-4 py-2 rounded-lg text-sm font-bold transition-all ${activeTab === 'my-classes' ? 'bg-indigo-600 text-white shadow-sm' : 'text-slate-500 hover:text-slate-700'}`}
                >
                    My Classes
                </button>
             </div>
            <button 
                ...
                className="px-4 py-2 bg-slate-900 text-white rounded-xl font-bold text-sm shadow-md hover:bg-slate-800 transition-colors flex items-center gap-2 whitespace-nowrap active:scale-95"
            >
      ...
      {activeTab === 'browse' && (
        <div className="space-y-6">
            <div className="bg-white border border-slate-200 rounded-xl p-3 flex items-center gap-2 max-w-md shadow-sm focus-within:ring-2 focus-within:ring-indigo-100 transition-all">
                <Search size={20} className="text-slate-400 ml-2" />
                <input 
                    placeholder="Search available subjects..." 
                    value={searchTerm}
                    onChange={(e) => setSearchTerm(e.target.value)}
                    className="w-full outline-none text-slate-900 placeholder:text-slate-400 text-sm bg-transparent" 
                />
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
                ...
                return (
                    <div key={subject.id} className="bg-white rounded-2xl border border-slate-200 overflow-hidden hover:shadow-xl hover:-translate-y-2 transition-all duration-300 group flex flex-col">
                    ...
                    <div className="p-6 pt-8 flex-1 flex flex-col">
                        <div className="flex justify-between items-start mb-2">
                            ...
                            {isJoined && <span className="bg-emerald-100 text-emerald-700 text-xs px-2 py-1 rounded-full font-bold flex items-center gap-1 animate-in zoom-in"><Check size={12} /> Joined</span>}
                        </div>
                        <p className="text-slate-500 text-sm mb-4 min-h-[40px]">{subject.description}</p>
                        
                        <div className="flex items-center gap-4 text-xs text-slate-400 font-medium mb-6 mt-auto">
                        ...
                        <button
                            ...
                            className="bg-slate-50 text-slate-700 border border-slate-200 py-2.5 rounded-xl font-bold text-sm hover:bg-slate-100 transition-colors flex justify-center items-center gap-2"
                        >
                        ...
                        {isJoined ? (
                            <button 
                            onClick={() => onStartLearning(subject)}
                            className="bg-slate-900 text-white py-2.5 rounded-xl font-bold text-sm hover:bg-slate-800 transition-colors flex justify-center items-center gap-2 hover:shadow-lg"
                            >
                            ...
                            className="border-2 border-indigo-600 text-indigo-600 py-2.5 rounded-xl font-bold text-sm hover:bg-indigo-50 transition-colors hover:shadow-md"
                            >
                            Join Class
                            </button>
                        )}
          ...
          <div className="space-y-10">
              ...
              <div>
                  <h3 className="text-xl font-bold text-slate-800 mb-4 flex items-center gap-2">
                      <School className="text-indigo-600" /> Teacher Classrooms
                  </h3>
                  {enrolledClassrooms.length === 0 ? (
                      <div className="text-center py-10 bg-slate-50 border-2 border-dashed border-slate-200 rounded-3xl">
                          <p className="text-slate-500 mb-4">You haven't joined any teacher classrooms yet.</p>
                          <button 
                            onClick={() => setShowJoinCodeModal(true)}
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
                          {enrolledClassrooms.map(room => (
                              <div key={room.id} className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden flex flex-col group hover:shadow-lg hover:-translate-y-1 transition-all duration-300">
                                  ...
                                  <div className="p-6 flex-1 flex flex-col">
                                      <p className="text-slate-500 text-sm mb-4 min-h-[40px]">{room.description || "No description."}</p>
                                      ...
                                      <button 
                                        ...
                                        className="w-full bg-slate-50 border border-slate-200 text-slate-600 font-bold py-3 rounded-xl hover:bg-indigo-600 hover:text-white hover:border-indigo-600 transition-colors flex justify-center items-center gap-2"
                                      >
              ...
              <div>
                  <h3 className="text-xl font-bold text-slate-800 mb-4 flex items-center gap-2">
                      <Brain className="text-indigo-600" /> Global Subjects
                  </h3>
                  {joinedGlobalSubjects.length === 0 ? (
                      <div className="text-center py-10 bg-slate-50 border-2 border-dashed border-slate-200 rounded-3xl">
                          <p className="text-slate-500 mb-4">You haven't started any learning paths yet.</p>
                          <button 
                            onClick={() => setActiveTab('browse')}
                            className="text-indigo-600 font-bold hover:underline"
                          >
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
                          {joinedGlobalSubjects.map(subject => (
                              <div key={subject.id} className="bg-white rounded-2xl border border-slate-200 overflow-hidden hover:shadow-lg hover:-translate-y-1 transition-all duration-300 flex flex-col group">
                                  ...
                                  <div className="p-6 flex-1 flex flex-col">
                                      <div className="mb-6 bg-slate-50 p-3 rounded-xl border border-slate-100">
                                          <div className="flex justify-between text-xs font-bold text-slate-500 uppercase tracking-wide mb-2">
                                              <span>Progress</span>
                                              <span className={subject.masteryLevel === 'Master' ? 'text-amber-500' : 'text-slate-700'}>{subject.progress}%</span>
                                          </div>
                                      ...
                                      <button 
                                          onClick={() => onStartLearning(subject)}
                                          className="w-full bg-slate-900 text-white py-3 rounded-xl font-bold text-sm hover:bg-slate-800 transition-colors flex justify-center items-center gap-2 mt-auto"
                                      >
        ...
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
           <div className="bg-white rounded-3xl w-full max-w-sm shadow-2xl p-6 animate-in zoom-in-95 duration-200">
               <div className="text-center mb-6">
                   ...
                   <p className="text-slate-500 text-sm">Enter the code shared by your teacher.</p>
               </div>
               ...
               <div className="mb-6">
                   <input 
                       ...
                       className="w-full text-center text-3xl font-mono font-bold tracking-widest p-4 rounded-xl border-2 border-slate-200 bg-white focus:border-indigo-500 focus:ring-4 focus:ring-indigo-100 outline-none uppercase text-slate-800"
                   />
      ...
      {showAssignmentsModal && selectedClassroomForAssignments && (
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
           <div className="bg-white rounded-3xl w-full max-w-lg shadow-2xl p-6 md:p-8 animate-in zoom-in-95 duration-200 flex flex-col max-h-[80vh]">
               ...
               <div className="overflow-y-auto pr-2 custom-scrollbar flex-1">
                   {classAssignments.length === 0 ? (
                       <div className="text-center py-12 border-2 border-dashed border-slate-200 rounded-2xl">
                           <div className="w-16 h-16 bg-slate-50 text-slate-300 rounded-full flex items-center justify-center mx-auto mb-4 text-slate-400">
                               <FileText size={24} />
                           </div>
                       ...
                       <div className="space-y-4">
                           {classAssignments.map(assign => (
                               <div key={assign.id} className="p-5 rounded-xl border border-slate-200 hover:border-indigo-200 bg-white shadow-sm transition-all group">
                                   <div className="flex justify-between items-start mb-2">
                                       <div className="flex items-center gap-2">
                                           <span className={`px-2 py-0.5 rounded-md text-[10px] font-bold uppercase tracking-wider
                                               ${assign.difficulty === 'Easy' ? 'bg-emerald-100 text-emerald-700' : 
                                                 assign.difficulty === 'Medium' ? 'bg-amber-100 text-amber-700' : 
                                   ...
                                   <h4 className="font-bold text-slate-800 text-lg mb-4 group-hover:text-indigo-700 transition-colors">
                                       {assign.topic}
                                   </h4>
                                   ...
                                   <div className="space-y-3">
                                       ...
                                       {assign.attachmentUrl && (
                                           <a 
                                             ...
                                             className="flex items-center justify-between p-3 rounded-lg bg-indigo-50 border border-indigo-100 hover:bg-indigo-100 transition-colors group/file"
                                           >
                                       ...
                                       <div className="flex gap-2">
                                            ...
                                            <button 
                                                ...
                                                className="flex-1 py-2.5 bg-slate-800 text-white rounded-lg font-bold text-sm hover:bg-slate-900 shadow-md transition-all flex items-center justify-center gap-2"
                                            >
                                                {assign.status === 'submitted' ? 'Retake Quiz' : 'Start Quiz'} <Play size={14} fill="currentColor" />
                                            </button>

                                            {/* Upload Submission */}
                                            <label className={`flex-1 py-2.5 border-2 border-dashed border-indigo-300 text-indigo-600 rounded-lg font-bold text-sm hover:bg-indigo-50 transition-all flex items-center justify-center gap-2 cursor-pointer ${submittingId === assign.id ? 'opacity-50' : ''}`}>
                                                <input 
                                                    type="file" 
                                                    accept=".pdf"
                                                    className="hidden" 
        ...
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
           <div className="bg-white rounded-3xl w-full max-w-lg shadow-2xl p-6 md:p-8 animate-in zoom-in-95 duration-200">
               <div className="flex justify-between items-center mb-6">
                 ...
                 <button onClick={() => setSelectedSubjectForJoin(null)} className="text-slate-400 hover:text-slate-600">
                   <X size={24} />
                 </button>
               </div>
               ...
               {step === 1 && (
                   <div className="animate-in slide-in-from-right-4 duration-300">
                        <div className="bg-indigo-50 p-4 rounded-xl border border-indigo-100 mb-6 flex gap-4">
                            <div className="bg-indigo-100 p-2 rounded-lg text-indigo-600 h-fit">
                                <Brain size={24} />
                            </div>
                            <div>
                                <h4 className="font-bold text-indigo-900 mb-1">Current Knowledge</h4>
                                <p className="text-indigo-700 text-sm">Help us tailor the starting point.</p>
                            </div>
                        </div>
                        ...
                        <div className="space-y-3">
                            {['Beginner', 'Intermediate', 'Advanced'].map((level) => (
                                <button 
                                    ...
                                    className={`w-full p-4 rounded-xl border-2 text-left transition-all ${assessmentLevel === level ? 'border-indigo-600 bg-indigo-50 ring-1 ring-indigo-500' : 'border-slate-200 hover:border-indigo-300'}`}
                                >
                                    <div className="flex justify-between items-center mb-1">
                                        <span className="font-bold text-slate-800">{level}</span>
                                        {assessmentLevel === level && <Check size={20} className="text-indigo-600" />}
                                    </div>
               ...
               {step === 2 && (
                   <div className="animate-in slide-in-from-right-4 duration-300">
                        <div className="bg-amber-50 p-4 rounded-xl border border-amber-100 mb-6 flex gap-4">
                            <div className="bg-amber-100 p-2 rounded-lg text-amber-600 h-fit">
                                <Target size={24} />
                            </div>
                            <div>
                                <h4 className="font-bold text-amber-900 mb-1">Set Your Mission</h4>
                                <p className="text-amber-800/80 text-sm">We'll build a custom roadmap to reach this specific goal.</p>
                            </div>
                        </div>

                        <label className="block text-sm font-bold text-slate-700 mb-3">What do you want to achieve?</label>
                        <div className="space-y-3">
                            {getGoalPresets(selectedSubjectForJoin.name).map((goal) => (
                                <button 
                                    ...
                                    className={`w-full p-3 rounded-xl border-2 text-left transition-all ${subjectGoal === goal ? 'border-amber-500 bg-amber-50' : 'border-slate-200 hover:border-amber-300'}`}
                                >
                            ...
                            <button 
                                ...
                                className={`w-full p-3 rounded-xl border-2 text-left transition-all ${subjectGoal === 'Custom' ? 'border-amber-500 bg-amber-50' : 'border-slate-200 hover:border-amber-300'}`}
                            >
                                <span className="font-medium text-slate-800 text-sm">Something else...</span>
                            </button>
                        </div>
                        ...
                        {subjectGoal === 'Custom' && (
                            <div className="mt-3">
                                <input 
                                    ...
                                    className="w-full px-4 py-3 rounded-xl border-2 border-amber-300 focus:ring-2 focus:ring-amber-200 outline-none"
                                    autoFocus
                                />
                            </div>
                        )}
                        ...
                        <div className="flex gap-3 mt-6">
                            <button 
                                onClick={() => setStep(1)}
                                className="px-6 py-4 text-slate-500 font-bold hover:text-slate-700"
                            >
                                Back
                            </button>
                            <button 
