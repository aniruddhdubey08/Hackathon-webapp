...
export const TeacherDashboard: React.FC<TeacherDashboardProps> = ({ userStats }) => {
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500 relative pb-12">
      {/* Header */}
      <div className="bg-white p-8 rounded-3xl border border-slate-200 shadow-sm mb-8 flex flex-col md:flex-row md:items-center justify-between gap-4">
         <div>
             <h2 className="text-3xl font-bold text-slate-900 mb-2">Hello, {userStats.name}!</h2>
             <p className="text-slate-500">Manage your classrooms, generate codes, and track student progress.</p>
         </div>
         <div className="flex bg-slate-100 p-1 rounded-xl">
             <button 
                onClick={() => setActiveTab('overview')}
                className={`px-6 py-2 rounded-lg font-bold text-sm transition-all ${activeTab === 'overview' ? 'bg-white text-indigo-600 shadow-sm' : 'text-slate-500 hover:text-slate-700'}`}
             >
                Overview
             </button>
             <button 
                onClick={() => setActiveTab('classrooms')}
                className={`px-6 py-2 rounded-lg font-bold text-sm transition-all ${activeTab === 'classrooms' ? 'bg-white text-indigo-600 shadow-sm' : 'text-slate-500 hover:text-slate-700'}`}
             >
                My Classrooms
             </button>
         </div>
        ...
        <>
            <div className="flex justify-between items-center mb-4">
                ...
                <button 
                    ...
                    className="px-4 py-2 bg-indigo-600 text-white rounded-lg text-sm font-medium hover:bg-indigo-700 flex items-center gap-2 shadow-lg shadow-indigo-200 transition-all"
                >
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                ...
                <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm hover:shadow-md transition-shadow">
                    <div className="flex justify-between items-start mb-4">
                        ...
                        <h3 className="text-3xl font-bold text-slate-800 mt-1">{assignments.length}</h3>
                        </div>
                ...
                <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm hover:shadow-md transition-shadow">
                    <div className="flex justify-between items-start mb-4">
                        ...
                        <h3 className="text-3xl font-bold text-slate-800 mt-1">{classrooms.length}</h3>
                        </div>
                        <div className="p-2 bg-emerald-50 text-emerald-600 rounded-lg"><School size={20} /></div>
                    </div>
                </div>
            </div>
            ...
            <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm mt-8">
                ...
                {assignments.length === 0 ? (
                    ...
                    <div key={assign.id} className="flex flex-col sm:flex-row sm:items-center justify-between p-4 border border-slate-100 rounded-xl hover:border-indigo-100 transition-colors gap-4">
                        <div className="flex items-center gap-4">
                            ...
                            <div>
                                <h4 className="font-bold text-slate-800">{assign.topic}</h4>
                                <div className="flex gap-3 text-xs text-slate-500 mt-1">
                                    ...
                                    {assign.attachmentUrl && <span className="flex items-center gap-1 text-indigo-600 font-medium bg-indigo-50 px-2 py-0.5 rounded"><FileText size={12} /> PDF Attached</span>}
                                </div>
                            </div>
                        </div>
                        <div className="flex items-center gap-3 sm:justify-end">
                            <button 
                                ...
                                className="px-4 py-2 bg-white border border-slate-200 text-slate-600 text-xs font-bold rounded-lg hover:border-indigo-500 hover:text-indigo-600 transition-colors flex items-center gap-2 shadow-sm"
                            >
      ...
      {activeTab === 'classrooms' && (
          <>
             ...
             <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                {classrooms.map(room => (
                    <div key={room.id} className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden flex flex-col hover:shadow-md transition-shadow">
                        ...
                        <div className="p-6 flex-1">
                            <p className="text-slate-500 text-sm mb-6 min-h-[40px]">{room.description || "No description provided."}</p>
                            
                            <div className="flex justify-between items-end">
                                <div>
                                    <span className="text-xs font-bold text-slate-400 uppercase tracking-wider block mb-1">Class Code</span>
                                    <button 
                                        onClick={() => copyToClipboard(room.code)}
                                        className="flex items-center gap-2 text-2xl font-mono font-bold text-slate-800 hover:text-indigo-600 transition-colors"
                                    >
                                        {room.code} 
                                        {copiedCode === room.code ? <Check size={20} className="text-emerald-500" /> : <Copy size={20} className="text-slate-300" />}
                                    </button>
                                </div>
                        ...
                        <div className="bg-slate-50 p-3 border-t border-slate-100 flex gap-2 justify-end">
                            <button 
                                onClick={() => handleViewStudents(room)}
                                className="text-xs font-bold text-slate-500 hover:text-indigo-600 px-3 py-1 flex items-center gap-1"
                            >
                                <Users size={14} /> View Students
                            </button>
                            <button className="text-xs font-bold text-slate-500 hover:text-indigo-600 px-3 py-1">Settings</button>
                        </div>
                    </div>
                ))}
                ...
                {classrooms.length === 0 && (
                     <div className="md:col-span-2 text-center py-16 bg-slate-50 border-2 border-dashed border-slate-200 rounded-3xl">
                         ...
                         <h3 className="text-lg font-bold text-slate-700 mb-1">No Classrooms Yet</h3>
                         <p className="text-slate-500 mb-6">Create a classroom to generate a code for your students.</p>
                         <button 
                            onClick={() => setShowCreateClassModal(true)}
                            className="px-6 py-3 bg-white border border-slate-300 text-slate-700 font-bold rounded-xl hover:bg-slate-100 transition-colors shadow-sm"
                        >
                            Create First Class
                        </button>
                     </div>
      ...
      {showCreateClassModal && (
        <div className="fixed inset-0 bg-slate-900/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
           <div className="bg-white rounded-3xl w-full max-w-lg shadow-2xl p-6 md:p-8 animate-in zoom-in-95 duration-200">
              <div className="flex justify-between items-center mb-6">
                <h2 className="text-2xl font-bold text-slate-800">Create New Classroom</h2>
                <button onClick={() => setShowCreateClassModal(false)} className="text-slate-400 hover:text-slate-600">
                  <X size={24} />
                </button>
              </div>
              ...
              <form onSubmit={handleCreateClass} className="space-y-4">
                 <div>
                    ...
                    <input 
                      ...
                      className="w-full px-4 py-3 rounded-xl border border-slate-200 bg-white focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 outline-none transition-all"
                      required
                    />
                 </div>
                 <div>
                    ...
                    <textarea 
                      ...
                      className="w-full px-4 py-3 rounded-xl border border-slate-200 bg-white focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 outline-none transition-all resize-none h-24"
                    />
                 </div>
        ...
        <div className="fixed inset-0 bg-slate-900/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
           <div className="bg-white rounded-3xl w-full max-w-lg shadow-2xl overflow-hidden animate-in zoom-in-95 duration-200 flex flex-col max-h-[80vh]">
              <div className="p-6 md:p-8 border-b border-slate-100 flex justify-between items-center bg-white sticky top-0 z-10">
                <div>
                    <h2 className="text-2xl font-bold text-slate-800">{selectedClassroomForStudents.name}</h2>
                    <p className="text-slate-500 text-sm">Enrolled Students ({studentList.length})</p>
                </div>
                <button onClick={() => setShowStudentListModal(false)} className="text-slate-400 hover:text-slate-600">
                  <X size={24} />
                </button>
              </div>
              ...
              <div className="overflow-y-auto p-6 flex-1">
                  {studentList.length === 0 ? (
                      <div className="text-center py-10">
                          <div className="bg-slate-100 rounded-full w-16 h-16 flex items-center justify-center mx-auto mb-4 text-slate-400">
                              <Users size={32} />
                          </div>
                          <p className="text-slate-500">No students have joined this class yet.</p>
                          <p className="text-sm text-slate-400 mt-2">Share code <span className="font-mono font-bold text-slate-600">{selectedClassroomForStudents.code}</span> with them.</p>
                      </div>
                  ) : (
                          ...
                          {studentList.map((student, idx) => (
                              <div key={idx} className="flex items-center gap-4 p-4 rounded-xl border border-slate-100 hover:bg-slate-50 transition-colors">
                                  <div className="shrink-0">
                                      {student.avatar ? (
                                          <img src={student.avatar} alt={student.name} className="w-12 h-12 rounded-full border border-slate-200" />
                                      ) : (
                                          <div className="w-12 h-12 rounded-full bg-indigo-100 flex items-center justify-center text-indigo-700 font-bold border border-slate-200">
                                  ...
                                  <div className="text-right">
                                      <span className="block text-sm font-bold text-indigo-600">Lvl {student.level}</span>
                                      <span className="text-xs text-slate-400">{student.totalXp.toLocaleString()} XP</span>
                                  </div>
                              </div>
                          ))}
                      </div>
              ...
              <div className="p-4 bg-slate-50 border-t border-slate-100 text-center">
                  <button 
                    ...
                    className="text-slate-500 font-bold hover:text-slate-700 text-sm"
                  >
                      Close
                  </button>
              </div>
      ...
      {showAssignModal && (
        <div className="fixed inset-0 bg-slate-900/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
           <div className="bg-white rounded-3xl w-full max-w-lg shadow-2xl p-6 md:p-8 animate-in zoom-in-95 duration-200">
              <div className="flex justify-between items-center mb-6">
                <h2 className="text-2xl font-bold text-slate-800">Assign Quiz / Worksheet</h2>
              ...
              {classrooms.length === 0 ? (
                  <div className="text-center py-6">
                      <p className="text-slate-500 mb-4">You need to create a classroom first!</p>
                      <button 
                ...
                <form onSubmit={handleCreateAssignment} className="space-y-4">
                    <div>
                        <label className="block text-sm font-semibold text-slate-700 mb-2">Select Classroom</label>
                        <select 
                            value={selectedClassroomId}
                            onChange={(e) => setSelectedClassroomId(e.target.value)}
                            className="w-full px-4 py-3 rounded-xl border border-slate-200 bg-white focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 outline-none"
                            required
                        >
                            {classrooms.map(c => (
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        ...
                        <input 
                            ...
                            className="w-full px-4 py-3 rounded-xl border border-slate-200 bg-white focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 outline-none text-slate-600"
                            required
                        />
                        </div>
                    </div>
                    ...
                    {/* PDF UPLOAD */}
                    <div className="border-2 border-dashed border-indigo-200 rounded-xl p-6 bg-indigo-50/50 text-center hover:bg-indigo-50 transition-colors">
      ...
      {showSubmissionsModal && selectedAssignmentForReview && (
        <div className="fixed inset-0 bg-slate-900/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
           <div className="bg-white rounded-3xl w-full max-w-2xl shadow-2xl overflow-hidden animate-in zoom-in-95 duration-200 flex flex-col max-h-[80vh]">
              <div className="p-6 md:p-8 border-b border-slate-100 flex justify-between items-center bg-white sticky top-0 z-10">
                <div>
                    <h2 className="text-2xl font-bold text-slate-800">{selectedAssignmentForReview.topic}</h2>
                    <p className="text-slate-500 text-sm">Submissions Review</p>
                </div>
              ...
              <div className="overflow-y-auto p-6 flex-1">
                  {selectedAssignmentForReview.submissions.length === 0 ? (
                      <div className="text-center py-10">
                          ...
                          <p className="text-slate-500">No students have submitted work yet.</p>
                      </div>
                  ) : (
                      <div className="space-y-4">
                          {selectedAssignmentForReview.submissions.map((sub, idx) => (
                              <div key={idx} className="flex flex-col sm:flex-row sm:items-center gap-4 p-4 rounded-xl border border-slate-100 hover:bg-slate-50 transition-colors">
                                  ...
                                  <div className="flex items-center gap-3">
                                      <div className="text-xs text-slate-500 px-3 py-1 bg-slate-100 rounded-lg truncate max-w-[150px]">
                                          {sub.fileName}
                                      </div>
                                      <a 
                                        href={sub.fileUrl} 
